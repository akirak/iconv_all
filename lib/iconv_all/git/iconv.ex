defmodule IconvAll.Git.Iconv do
  ...
  def convert_file_tmp(file, source_encoding, target_encoding) do
    with {:ok, tmp} <- Briefly.create(),
         {:ok, _} <- convert_file(file, tmp, source_encoding, target_encoding),
         do: {:ok, tmp}
  end

  @spec convert_file(Path.t(), Path.t(), String.t(), String.t()) ::
          {:ok, nil} | {:error, %{reason: String.t()}}
  defp convert_file(source_file, target_file, source_encoding, target_encoding) do
    case System.cmd("iconv", [
           "-f",
           source_encoding,
           "-t",
           target_encoding,
